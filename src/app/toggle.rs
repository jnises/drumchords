...
pub fn toggle(ui: &mut egui::Ui, on: &mut bool, text: &str) {
    ...
    let disabled_color = egui::lerp(
        Rgba::from(visuals.bg_fill)..=Rgba::from(visuals.fg_stroke.color),
        0.5,
    );
    let text_color =
        egui::lerp(disabled_color..=Rgba::from(visuals.fg_stroke.color), how_on).into();
